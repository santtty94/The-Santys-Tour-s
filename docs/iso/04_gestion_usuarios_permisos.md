...
La gestión de usuarios se aplica en los dos sistemas del proyecto piloto:

| Sistema | Plataforma | Acceso para ejecutar comandos |
|---------|-----------|-------------------------------|
| Servidor Ubuntu Server 22.04 LTS | AWS Academy (EC2 t3.micro) | PuTTY desde Windows 11 Pro (VirtualBox) |
| Cliente Windows 11 Pro | VirtualBox (Lenovo Legion 5) | Directamente en la VM |

---

## 2. PARTE A — Usuarios y permisos en el servidor Ubuntu (AWS Academy)

> Conectarse primero al servidor mediante PuTTY con la sesión guardada `SantysTours-Server` y la clave `labsuser.ppk` (vockey).

### 2.1 Planificación de usuarios

| Usuario | Rol | Nivel de acceso |
|---------|-----|-----------------|
| `ubuntu` | Usuario por defecto de AWS Academy | sudo completo — creado automáticamente por AWS |
...
### 2.8 Resumen de permisos por usuario en el servidor
...
1. Explorador de archivos → barra de dirección → `\\IP_PUBLICA_AWS`
